...
function PreviewMeeting({formValue}) {
    ...
    const [selectedTime, setSelectedTime] = useState(null); // Added to keep track of selected time slot
    useEffect(()=>{
            ...
            <div className='md:col-span-2 flex px-4'>
                ...
                >
                    {timeSlots?.map((time,index)=>(
                        <Button 
                        onClick={() => setSelectedTime(time)} // Added to handle time slot selection
                        className={`border-primary
                         text-primary ${selectedTime === time ? 'bg-primary text-white' : ''}`} // Added to highlight selected time slot
